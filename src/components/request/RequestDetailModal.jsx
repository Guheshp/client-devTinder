...
import {
    BsBriefcase, BsInfoCircle, BsX, BsPatchCheckFill, BsGem,
    BsLink45Deg, BsGeoAlt, BsCheckCircleFill, BsXCircleFill,
    BsEnvelopeAt, BsGenderAmbiguous, BsCalendarEvent, BsCodeSlash
} from 'react-icons/bs';
import { FaGithub, FaLinkedin, FaTwitter, FaGlobe } from 'react-icons/fa';
import { DEFAULT_IMG, skillList } from '../../utils/helper/constant';

const RequestDetailModal = ({ user, requestId, onClose, onReview }) => {
    ...
    // Helper to get readable name from ID
    const getSkillName = (skillId) => {
        const foundSkill = skillList.find((s) => s.id === skillId);
    ...
    return (
        <dialog ref={modalRef} id="request-detail-modal" className="modal modal-bottom sm:modal-middle backdrop-blur-sm">
            <div className="modal-box w-11/12 max-w-2xl bg-base-100 p-0 overflow-hidden relative custom-scrollbar shadow-2xl">
                ...
                <div className="bg-primary/5 p-6 relative shrink-0 border-b border-base-200">
                    <button onClick={handleClose} className="btn btn-sm btn-circle btn-ghost absolute right-3 top-3 hover:bg-base-200 z-10"><BsX className="text-2xl" /></button>
                ...
                <div className="p-6 overflow-y-auto max-h-[60vh] space-y-6">
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {skills?.length > 0 ? skills.map((skillId, i) => (
                                <span key={i} className="badge badge-lg badge-outline py-4 px-4 hover:bg-primary hover:text-white cursor-default select-none">
                ...
                <div className="p-4 bg-base-100 border-t border-base-200 flex justify-end gap-3 shrink-0">
                    <button
                        ...
                        onClick={() => {
                            onReview("rejected", requestId); // Status: "rejected"
                            handleClose();
                        }}
                    >
                    ...
                    <button
                        ...
                        onClick={() => {
                            onReview("accepeted", requestId); // Status: "accepeted" (Matches your DB typo)
                            handleClose();
                        }}
                    >
